...
generateForm.addEventListener('submit', async function (e) {
    ...
    // Add style modifiers to the prompt to get a consistent logo style
    const styledPrompt = `vector logo of ${prompt}, minimalist, clean, 2d, flat, professional, behance, dribbble`;
    ...
    const poll = async () => {
        try {
            ...
            if (statusResult.status === 'IN_PROGRESS' || statusResult.status === 'IN_QUEUE') {
                ...
                loadingDiv.textContent = `Generating logo... (${progress}%)`;
